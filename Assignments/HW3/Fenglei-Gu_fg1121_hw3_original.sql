...
FROM gaokao
WHERE CollegeName GLOB '*一流*建设高校*' /*Offered by Minitry of Education of China, there are totally two types of first-class honor suffix: '一流大学建设高校' or in English First-Class University, and '一流学科建设高校' or in English First-Class College*/
ORDER BY u ASC
LIMIT 5;
...
SELECT 'Q8';
SELECT TRIM(CollegeName,'(一流%建设高校)') AS c, MIN(LeastScore) AS m
FROM gaokao
GROUP BY c
...
-- 11. List least admission scores for medicine majors (with major name including medicine) at all universities
SELECT 'Q11';
SELECT CollegeName,MajorName,LeastScore
FROM gaokao
WHERE MajorName LIKE '%医学%' /*'医学' means medicine in Chinese. Universities in China offer medicine major for undergrad*/
